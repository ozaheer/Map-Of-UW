...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    /**
     * Path objects store a discovered path of vertices and the overall distance or cost
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            List<T> copyOfSourceList = new LinkedList<>();
            copyOfSourceList.addAll(source.dataSequence);
            this.dataSequence = copyOfSourceList;
            this.end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        PriorityQueue<Path> unvisitedQueue = new PriorityQueue<Path>();
        Hashtable<Vertex, Integer> visitedNodes = new Hashtable<Vertex, Integer>();
        Path shortestPath = null;

        if(start == null || end == null || !containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("Vertices containing start/end cannot be found");
        } else {
            Path current;
            unvisitedQueue.add(new Path(vertices.get(start))); //add initial vertex/distance pair into queue

            while (!unvisitedQueue.isEmpty()) { //run until queue runs out (ie all paths have been tried)
            current = unvisitedQueue.poll(); // pop the next element out of the queue
            visitedNodes.put(current.end, current.distance); //adds that next element to visited list

            // handles shortestPath stuff, when paths are reaching the end node
            if (current.start.data.equals(start) && current.end.data.equals(end)) {
                if (shortestPath != null) {
                    if (current.compareTo(shortestPath) == -1) {
            ...
            //loop clones current path, extends it, and adds it to queue (and does it for all edges connected to end vertex)
            for (int i = 0; i < current.end.edgesLeaving.size(); i++) {
                boolean vertexAlreadyVisited = false;
                for (int j = 0; j < visitedNodes.size(); j++) { //checks if adjacent nodes have been visited yet
                if (visitedNodes.containsKey(current.end.edgesLeaving.get(i).target)) {
                    vertexAlreadyVisited = true;
                    break;
                }
            }
                if (vertexAlreadyVisited) { //don't add that path to the unvisitedQueue
                    if (visitedNodes.get(current.end.edgesLeaving.get(i).target) < 
                    current.distance + current.end.edgesLeaving.get(i).weight) {    // do a comparison to distance of path that visits this node
                    continue;
                } else { //replaces old distance in visitedNodes hashtable with new shortest distance for specified path
                    visitedNodes.remove(current.end.edgesLeaving.get(i).target);
                }
            }
                Path pathToAdd = new Path(current);
                pathToAdd.extend(pathToAdd.end.edgesLeaving.get(i));
                unvisitedQueue.add(pathToAdd);
            }
